...
describe("parseSkills", () => {
  test("parses skill ids and names with no perms", async () => {
    ...
    expect(skills.find((s) => s.id === 1)).toMatchObject({
      id: 1,
      name: "Liver of Steel",
      perm: "none",
    });
    expect(skills.find((s) => s.id === 4006)).toMatchObject({
      id: 4006,
  ...
  test("parses hardcore permed skills — (<b>HP</b>) marker", async () => {
    ...
    expect(skills.find((s) => s.id === 1005)).toMatchObject({
      id: 1005,
      perm: "hardcore",
    });
  });
  ...
  test("non-permed skills remain none alongside permed ones", async () => {
    ...
    expect(skills.find((s) => s.id === 15)).toMatchObject({
      id: 15,
      perm: "none",
    });
  });
