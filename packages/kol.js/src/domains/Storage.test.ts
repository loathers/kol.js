...
describe("pull", () => {
  test("returns success when item moved from storage to inventory", async () => {
    vi.spyOn(client, "fetchText").mockResolvedValueOnce(
      ajaxResponse(
        "<b>seal-clubbing club (1)</b> moved from storage to inventory",
      ),
    );
  ...
  test("accepts an Item object", async () => {
    ...
    expect(await storage.pull({ id: 1 } as never, 1)).toStrictEqual({
      success: true,
    });
  });
  ...
  test("returns item not in storage", async () => {
    ...
    expect(await storage.pull(1, 1)).toStrictEqual({
      success: false,
      reason: "Item not in storage",
    });
  });
  ...
  test("returns daily pull limit reached", async () => {
    ...
    expect(await storage.pull(1, 1)).toStrictEqual({
      success: false,
      reason: "Daily pull limit reached",
    });
  });
...
describe("pullMeat", () => {
  ...
  test("returns insufficient meat when not enough in storage", async () => {
    ...
    expect(await storage.pullMeat(1000)).toStrictEqual({
      success: false,
      reason: "Insufficient meat in storage",
    });
  });
});
