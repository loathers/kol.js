...
const MODIFIERS = {
  muscle: {
    ...
    format: (roll) => `Muscle +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  mysticality: {
    roll: (level, rng) => rng.mtRand.roll(10 * level + 10, 12 * level + 12),
    format: (roll) => `Mysticality +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  moxie: {
    roll: (level, rng) => rng.mtRand.roll(10 * level + 10, 12 * level + 12),
    format: (roll) => `Moxie +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  resistanceHot: {
  ...
  maximumHp: {
    ...
    format: (roll) => `Maximum HP +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  maximumMp: {
    roll: (level, rng) => rng.mtRand.roll(20 * level + 10, 20 * level + 30),
    format: (roll) => `Maximum MP +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  hpRegen: {
  ...
  damageReduction: {
    ...
    format: (roll) => `Damage Reduction: ${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  damageAbsorption: {
    roll: (level, rng) => rng.mtRand.roll(10 * level + 10, 15 * level + 15),
    format: (roll) => `Damage Absorption +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  itemDrop: {
    roll: (level, rng) => rng.mtRand.roll(5 * level + 3, 5 * level + 8),
    format: (roll) => `+${roll as number}% Item Drops From Monsters`,
    alwaysExists: false,
    order: 0,
  },
  meatDrop: {
    roll: (level, rng) => rng.mtRand.roll(10 * level + 5, 10 * level + 15),
    format: (roll) => `+${roll as number}% Meat From Monsters`,
    alwaysExists: false,
    order: 0,
  },
  monsterLevel: {
    roll: (level, rng) => rng.mtRand.roll(5 * level + 0, 5 * level + 10),
    format: (roll) => `+${roll as number} to Monster Level`,
    alwaysExists: false,
    order: 0,
  },
  damageHot: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Hot Damage`,
    alwaysExists: true,
    order: 0,
  },
  damageCold: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Cold Damage`,
    alwaysExists: true,
    order: 0,
  },
  damageStench: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Stench Damage`,
    alwaysExists: true,
    order: 0,
  },
  damageSleaze: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Sleaze Damage`,
    alwaysExists: true,
    order: 0,
  },
  damageSpooky: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Spooky Damage`,
    alwaysExists: true,
    order: 0,
  },
  spellDamageHot: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Damage to Hot Spells`,
    alwaysExists: true,
    order: 0,
  },
  spellDamageCold: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Damage to Cold Spells`,
    alwaysExists: true,
    order: 0,
  },
  spellDamageStench: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Damage to Stench Spells`,
    alwaysExists: true,
    order: 0,
  },
  spellDamageSleaze: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Damage to Sleaze Spells`,
    alwaysExists: true,
    order: 0,
  },
  spellDamageSpooky: {
    roll: (level, rng) => rng.mtRand.roll(4 * level + 4, 6 * level + 6),
    format: (roll) => `+${roll as number} Damage to Spooky Spells`,
    alwaysExists: true,
    order: 0,
  },
  familiarWeight: {
    roll: (level, rng) => 7 + 2 * level - rng.mtRand.roll(0, 2),
    format: (roll) => `+${roll as number} to Familiar Weight`,
    alwaysExists: true,
    order: 0,
  },
  familiarDamage: {
    roll: (level, rng) => rng.mtRand.roll(15 * level + 15, 25 * level + 25),
    format: (roll) => `Familiar Damage +${roll as number}`,
    alwaysExists: true,
    order: 0,
  },
  familiarExperience: {
    roll: (level) => level + 1,
    format: (roll) => `+${roll as number} Familiar Experience per Combat`,
    alwaysExists: false,
    order: 0,
  },
