...
export class Clan {
  ...
  async getInfo(clanId: number): Promise<ClanInfo | null> {
    ...
    const name =
      html.match(/<b style="color: white">([^<]+)<\/b>/)?.[1] ?? null;
    const leaderMatch = html.match(
      /href="showplayer\.php\?who=(\d+)">([^<]+)<\/a><\/b>, Level/,
    );
    ...
    const pageOptions = [
      ...html.matchAll(/<option[^>]*>(\d+) - (\d+)<\/option>/g),
    ];
    const memberCount =
      pageOptions.length > 0
        ...
        : [
            ...html.matchAll(
              /<a class=nounder href="showplayer\.php\?who=\d+">/g,
            ),
          ].length;
    ...
    const trophies = Object.fromEntries(
      [
        ...html.matchAll(
          /desc_clantrophy\.php[^<]*<\/td><td valign=center><b>([^<]+)<\/b> \((\d+)\)/g,
        ),
  ...
  async transferLeadership(playerId: number): Promise<Result> {
    ...
    if (result.includes("Leadership of clan transferred"))
      return { success: true };
    if (result.trim() === "")
      return { success: false, reason: "Insufficient clan admin rights" };
  ...
  async getRanks(): Promise<ClanRank[]> {
    ...
    const orChecked = (block: string, prefix: string) =>
      [
        ...block.matchAll(
          new RegExp(`name=${prefix}\\d+[^>]*value=(\\d+) checked`, "g"),
        ),
      ].reduce<number>((acc, [, v]) => acc | Number(v), 0);
    const namedRanks = [
      ...html.matchAll(
        /<form action=clan_editranks\.php[^>]*>([\s\S]*?)<\/form>/g,
      ),
    ].map(([, block]) => ({
      ...
      karmaLimit: Number(
        block.match(/name=karmalimit value="(\d+)"/)?.[1] ?? 0,
      ),
      zeroKarmaLimit: Number(
        block.match(/name=zerokarmalimit value="(\d+)"/)?.[1] ?? 0,
      ),
    }));
    return [
      ...namedRanks,
  ...
  async setPlayerRank(
    ...
    if (result.includes("Modifications made:</td>"))
      return { success: false, reason: "Player not in clan" };
    return { success: false, reason: "Unknown" };
  }
